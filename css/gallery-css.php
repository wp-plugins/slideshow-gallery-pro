...
<?php endforeach; ?>
<?php IF ($styles['width_temp']) {
	$styles['width'] = $styles['width_temp'];
}?>
<?php IF ($styles['height_temp']) {
...
#fullsize { position:relative; z-index:1; overflow:hidden; width:<?php echo ((int) $styles['width'] - 6); ?>px; 
...
#image { margin-top:-20px; width:<?php echo ((int) $styles['width'] - 6);?>px; }
...
<?php else : ?>
	#image img { position:left; border:none; width:auto; }<?php endif; ?> 
